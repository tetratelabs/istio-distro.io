...
will be triggered to push the built binaries, `manifest.json`, and `download.sh` to [Tetrate Istio Distro's repository on Cloudsmith](https://dl.getistio.io/public/raw/files). Please note that `manifest.json` and `download.sh` are not tagged at Bintray level, so they are overwritten by the new revision. The reason for that is because it is convenient to have "static" URLs for these two resources.
